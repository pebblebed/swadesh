...
Currently implemented:
  ...
  - deferred_*   : recognized but not yet converted (Slavic orthography, native
                   scripts, low-resource Latin) -> ipa left empty for later tiers.
...
def main():
    ...
    with open(SUMMARY, "w", encoding="utf-8", newline="\n") as f:
        ...
        for (m, c), n in sorted(conf_count.items()):
            f.write(f"{m}\t{c}\t{n}\n")
        f.write("\n# Americanist lists: residual (unconverted) symbols still present\n")
    ...
    print(f"\nIPA populated now : {conv} records "
          f"({methods['americanist']} via Americanist Tier 1)")
